...
import { movies, singleMovie, responses } from '../../testUtils/onboardingListData'
...
describe('OnboardingList', () => {
  describe('Flatlist', () => {
    it('renders list items', async () => {
      const { queryByTestId, getByText } = renderWithRedux(<OnboardingList {...props} />, { movies: singleMovie, moviesList: [] })
    ...
    it('has button disabled', () => {
      const { queryByTestId } = renderWithRedux(<OnboardingList {...props} />, { movies: singleMovie, moviesList: [] })
      expect(queryByTestId('sendButton')).toBeDisabled()
    })

    it('has element picked', async () => {
      const { getByTestId, getByText } = renderWithRedux(<OnboardingList {...props} />, { movies: singleMovie, moviesList: [] })
    ...
    it('has button enabled after three selections', async () => {
      const { queryByTestId, getAllByTestId, getByText } = renderWithRedux(<OnboardingList {...props} />, { movies, moviesList: [] })
      const [element1, element2, element3, checkboxes] = await waitForElement(() => [
